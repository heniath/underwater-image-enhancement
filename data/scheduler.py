...
class GradualWarmupScheduler(_LRScheduler):
    ...
    def get_lr(self):
        ...
        if self.multiplier == 1.0:
            return [
                base_lr * (self.last_epoch / self.total_epoch)
    ...
    def step_ReduceLROnPlateau(self, metrics, epoch=None):
        ...
        if self.last_epoch <= self.total_epoch:
            ...
            self.after_scheduler.step(metrics, None if epoch is None
                                      else epoch - self.total_epoch)

    def step(self, epoch=None, metrics=None):
        if not isinstance(self.after_scheduler, ReduceLROnPlateau):
            if self.finished and self.after_scheduler:
                self.after_scheduler.step(None if epoch is None
                                          else epoch - self.total_epoch)
            else:
                super().step(epoch)
        else:
...
class CosineAnnealingRestartCyclicLR(_LRScheduler):
    ...
    def __init__(self, optimizer, periods, restart_weights=(1,),
        ...
        self.cumulative_period = [
            sum(periods[:i + 1]) for i in range(len(periods))
        ]
