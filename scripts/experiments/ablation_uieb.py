...
def _make_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--color_weight",         type=float, default=0.0,
                   help="Cosine color angle loss weight.")
    p.add_argument("--wavelet_weight",       type=float, default=0.0,
                   help="Wavelet high-frequency detail loss weight.")
    p.add_argument("--use_charbonnier",      action="store_true",
...
def train_one_run(
    ...
    for epoch in range(1, args.nEpochs + 1):
        ...
        tr_loss, _ = train_epoch(
            ...
            criterion,
            device,
            desc=f"[{variant}|s{seed}] Ep {epoch}/{args.nEpochs}",
        )
        vl_loss = val_loss_epoch(
            ...
            criterion,
            device,
            desc=f"[{variant}|s{seed}] Val {epoch}/{args.nEpochs}",
        )
...
def eval_checkpoint(
    ...
    metrics, n = evaluate_loader(model, loader, device, desc=f"[{model_name}] Eval Test")
        ...
        for variant in args.variants:
            ...
            for seed_idx, (seed, bp) in enumerate(
                ...
                try:
                    ...
                    cleanup_system(device, verbose=False)
