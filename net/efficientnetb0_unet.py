...
except ModuleNotFoundError:                       # run directly: `python net/efficientnetb0_unet.py`
    import os, sys
...
class EfficientNetB0UNet(nn.Module):
    ...
    Example::
        ...
        model = EfficientNetB0UNet(in_channels=5)
...
# ---------------------------------------------------------------------------
# Smoke-test: `python net/efficientnetb0_unet.py`
# ---------------------------------------------------------------------------
if __name__ == "__main__":
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print("=" * 55)
    print(f"  EfficientNetB0UNet smoke-test  –  device: {device}")
    print("=" * 55)

    for in_channels in (3, 4, 5):
        model = EfficientNetB0UNet(in_channels=in_channels, pretrained=False).to(device)
        model.eval()
