...
_BACKBONE_VARIANTS = {
    ...
    "uwlytmsv2": _LEGACY_VARIANTS,
    "uwlytv2": _V2_VARIANTS,
    "uwlytv2tiny": _V2_VARIANTS,
    "nafnet": _LEGACY_VARIANTS,
    "nafnettiny": _LEGACY_VARIANTS,
    "nafnetmicro": _LEGACY_VARIANTS,
    "fanet": _LEGACY_VARIANTS,
    "mobileie": _LEGACY_VARIANTS,
    "liteenhancenet": _LEGACY_VARIANTS,
...
def build_model(name: str, pretrained_backbone: bool = False) -> nn.Module:
    ...
    if backbone == "nafnettiny":
        from .nafnet import build_nafnettiny

        return build_nafnettiny(in_channels=in_channels)

    if backbone == "nafnetmicro":
        from .nafnet import build_nafnetmicro

        return build_nafnetmicro(in_channels=in_channels)

    if backbone == "nafnet":
        from .nafnet import build_nafnet

        return build_nafnet(in_channels=in_channels)

    if backbone == "fanet":
        from .fanet import build_fanet

        return build_fanet(in_channels=in_channels)

    if backbone == "fanetplus":
        from .fanetplus import build_fanetplus

        return build_fanetplus(in_channels=in_channels)

    if backbone == "mobileie":
        from .mobileie import build_mobileie

        return build_mobileie(in_channels=in_channels)

    if backbone == "liteenhancenet":
        from .liteenhancenet import build_liteenhancenet

        return build_liteenhancenet(in_channels=in_channels)

    if backbone == "lsnet":
        from .lsnet import build_lsnet

        return build_lsnet(in_channels=in_channels)
    ...
    from .uwlyt import build_uwlyt, build_uwlytms, build_uwlytmsv2, build_uwlytv2
    ...
    if backbone == "uwlytmsv2":
        return build_uwlytmsv2(in_channels)
