"""MBConv U-Net++ with nested dense skip connections and deep supervision."""

import torch
import torch.nn as nn
import torch.nn.functional as F

from .mbconv_unet import MobileDoubleConv


class MBConvUNetPlusPlusDefined(nn.Module):
    """U-Net++ topology using MBConv blocks from ``mbconv_unet``."""
    ...
    def __init__(
        ...
        self.conv0_0 = MobileDoubleConv(in_channels, f0)
        self.conv1_0 = MobileDoubleConv(f0, f1)
        self.conv2_0 = MobileDoubleConv(f1, f2)
        self.conv3_0 = MobileDoubleConv(f2, f3)
        self.conv4_0 = MobileDoubleConv(f3, f4)

        self.conv0_1 = MobileDoubleConv(f0 + f1, f0)
        self.conv1_1 = MobileDoubleConv(f1 + f2, f1)
        self.conv2_1 = MobileDoubleConv(f2 + f3, f2)
        self.conv3_1 = MobileDoubleConv(f3 + f4, f3)

        self.conv0_2 = MobileDoubleConv(f0 * 2 + f1, f0)
        self.conv1_2 = MobileDoubleConv(f1 * 2 + f2, f1)
        self.conv2_2 = MobileDoubleConv(f2 * 2 + f3, f2)

        self.conv0_3 = MobileDoubleConv(f0 * 3 + f1, f0)
        self.conv1_3 = MobileDoubleConv(f1 * 3 + f2, f1)

        self.conv0_4 = MobileDoubleConv(f0 * 4 + f1, f0)
