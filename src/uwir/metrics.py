...
def evaluate_loader(
    ...
    if max_samples and hasattr(loader, "batch_size") and loader.batch_size:
        total_steps = min(total_steps, int(np.ceil(max_samples / loader.batch_size)))

    model_param = next((p for p in model.parameters() if p.is_floating_point()), None)
    target_dtype = model_param.dtype if model_param is not None else torch.float32

    pbar = tqdm(loader, desc=desc, total=total_steps, leave=False, dynamic_ncols=True)
    for inp, gt in pbar:
        inp = inp.to(device=device, dtype=target_dtype)
        ...
        pred_np = pred.float().cpu().permute(0, 2, 3, 1).numpy().clip(0, 1)
        gt_np = gt.float().cpu().permute(0, 2, 3, 1).numpy().clip(0, 1)
        ...
        if results["psnr"]:
            pbar.set_postfix({"psnr": f"{results['psnr'][-1]:.2f}dB", "ssim": f"{results['ssim'][-1]:.3f}"})
