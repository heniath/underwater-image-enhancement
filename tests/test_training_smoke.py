...
def test_tournament_models_training_step(model_name):
    ...
    train_loss, _ = train_epoch(model, _loader(channels=spec.in_channels), optimizer, criterion, torch.device("cpu"))
    val_loss = val_loss_epoch(model, _loader(channels=spec.in_channels), criterion, torch.device("cpu"))
