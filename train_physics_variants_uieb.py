...
class PhysicsCompositeLoss(nn.Module):
    ...
    Composite Loss supporting:
      1. 'vgg_reduced'   : 1.0 * L1 + vgg_weight * VGG (+ lambda_redeg * Redeg if 5ch) (User proposed optimal)
      2. 'vgg_redeg'     : 1.0 * L1 + 1.0 * VGG + lambda_redeg * Redeg
      3. 'vgg'           : 1.0 * L1 + 1.0 * VGG
      4. 'compound_redeg': 1.0 * L1 + 0.5 * MSE + 0.2 * SSIM + lambda_redeg * Redeg
    """
    def __init__(self, target_device, loss_mode: str = "vgg_reduced", vgg_weight: float = 0.1, lambda_redeg: float = 0.1):
        super().__init__()
    ...
    def forward(self, pred: torch.Tensor, target: torch.Tensor, i_redeg: torch.Tensor = None, raw_rgb: torch.Tensor = None) -> Tuple[torch.Tensor, dict]:
        ...
        if self.loss_mode == "vgg_reduced":
            with torch.amp.autocast('cuda', enabled=False):
                vgg_val = self.vgg(pred.float(), target.float())
            total = 1.0 * l1_val + self.vgg_weight * vgg_val
            comps["vgg"] = float(vgg_val.item())
        elif self.loss_mode in ["vgg_redeg", "vgg"]:
            with torch.amp.autocast('cuda', enabled=False):
                vgg_val = self.vgg(pred.float(), target.float())
...
def run_variant(variant_name: str, in_channels: int, train_pairs, val_pairs, test_configs, args, device):
    ...
    ch_tag = f"_c{args.base_channels}" if args.base_channels != 24 else ""
    loss_tag = (f"vgg{args.vgg_weight}_redeg" if use_redeg else f"vgg{args.vgg_weight}") + ch_tag

    if args.loss_mode == "vgg_reduced":
        loss_desc = f"1.0*L1 + {args.vgg_weight}*VGG" + (f" + {args.lambda_redeg}*Redeg" if use_redeg else "")
    elif args.loss_mode == "compound_redeg":
        loss_desc = "1.0*L1 + 0.5*MSE + 0.2*SSIM" + (f" + {args.lambda_redeg}*Redeg" if use_redeg else "")
    else:
        loss_desc = "1.0*L1 + 1.0*VGG" + (f" + {args.lambda_redeg}*Redeg" if use_redeg else "")

    print("\n" + "=" * 90, flush=True)
    print(f" KHOI CHAY HUAN LUYEN: {variant_name.upper()} (in_channels={in_channels}, base_ch={args.base_channels})", flush=True)
    print(f" Loss Function: {loss_desc}", flush=True)
    ...
    model = PhysicsOSANet(in_channels=in_channels, out_channels=3, base_channels=args.base_channels, use_fgdpa=True).to(device)
    total_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"Loaded Model: PhysicsOSANet ({total_params:,} params / {total_params/1e3:.2f}k)", flush=True)

    criterion = PhysicsCompositeLoss(target_device=device, loss_mode=args.loss_mode, vgg_weight=args.vgg_weight, lambda_redeg=args.lambda_redeg if use_redeg else 0.0)

    optimizer = torch.optim.Adam(model.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    if args.scheduler == "cosine":
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=args.epochs, eta_min=1e-6)
    else:
        scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[int(args.epochs * 0.5), int(args.epochs * 0.75)], gamma=0.5)
    ...
    for idx, r in enumerate(bench_results):
        ...
        csv_rows.append(f"{idx+1 if idx==0 else ''},{v_title},\"{arch_t}\",{r['Dataset']},\"{loss_t}\",{b_val},{r['PSNR']:.3f},{r['SSIM']:.4f},{r['CIEDE2000']:.3f},{r['UCIQE']:.4f},{r['UIQM']:.4f},{p_cnt},{f_cnt},{args.batch_size},{args.epochs},{r['Latency_ms']:.2f} ms,{t_time},{t_dset},{hw}\n")
    ...
    print(f"--> Da luu ket qua vao: {out_csv}", flush=True)

    # Append to master benchmark_results.csv
    if RESULTS_CSV.exists():
        try:
            with open(RESULTS_CSV, "r", encoding="utf-8") as f:
                existing_lines = f.readlines()
            existing_indices = [int(line.split(",")[0]) for line in existing_lines if line and line[0].isdigit() and line.split(",")[0].isdigit()]
            next_idx = (max(existing_indices) + 1) if existing_indices else 1
            master_rows = []
            for idx, line in enumerate(csv_rows):
                parts = line.split(",", 1)
                if idx == 0:
                    master_rows.append(f"{next_idx},{parts[1]}")
                else:
                    master_rows.append(f",{parts[1]}")
            with open(RESULTS_CSV, "a", encoding="utf-8") as f:
                f.writelines(master_rows)
            print(f"--> Da cap nhat ket qua vao master CSV: {RESULTS_CSV}\n", flush=True)
        except Exception as e:
            print(f"--> [Warning] Khong the cap nhat master CSV: {e}\n", flush=True)
...
def main():
    ...
    parser.add_argument("--loss_mode", type=str, default="vgg_reduced", choices=["vgg_reduced", "compound_redeg", "vgg_redeg", "vgg"], help="Loss mode: vgg_reduced (1.0*L1+vgg_weight*VGG+0.1*Redeg), compound_redeg, vgg_redeg, hoac vgg")
    parser.add_argument("--vgg_weight", type=float, default=0.1, help="Trong so VGG perceptual loss trong vgg_reduced mode (mac dinh: 0.1)")
    parser.add_argument("--lambda_redeg", type=float, default=0.1, help="Trong so re-degradation optical loss (mac dinh: 0.1)")
    parser.add_argument("--epochs", type=int, default=200, help="So epoch huan luyen (mac dinh: 200)")
    parser.add_argument("--batch_size", type=int, default=16, help="Kich thuoc batch (mac dinh: 16, hoac 8)")
    parser.add_argument("--base_channels", type=int, default=24, help="So channel co ban cho PhysicsOSANet (mac dinh: 24 ~25k params, 36 ~53k params, 40 ~65k params)")
    parser.add_argument("--crop_size", type=int, default=256, help="Kich thuoc anh (mac dinh: 256)")
    parser.add_argument("--lr", type=float, default=1e-4, help="Learning rate khoi dau (mac dinh: 1e-4)")
    parser.add_argument("--weight_decay", type=float, default=1e-5, help="Weight decay cho Adam (mac dinh: 1e-5)")
    parser.add_argument("--scheduler", type=str, default="cosine", choices=["step", "cosine"], help="LR Scheduler: cosine hoac step")
    ...
    print(f" Setting  : Variant={args.variant} | BaseCh={args.base_channels} | Epochs={args.epochs} | Batch={args.batch_size} | LR={args.lr} ({args.scheduler}) | Loss={args.loss_mode} (vgg_w={args.vgg_weight}, Redeg={args.lambda_redeg})", flush=True)
    print("=" * 105, flush=True)
